...
const { verifyToken, verifyAdmin, setMinWorkerTimestamp } = require('../middleware/auth');
...
// Force logout all clinic workers (Admin only)
router.post('/logout-all-workers', verifyToken, verifyAdmin, async (req, res) => {
  try {
    const GlobalSetting = require('../models/GlobalSetting');
    const now = Date.now();
    await GlobalSetting.findOneAndUpdate(
      { key: 'minWorkerTokenIssuedAt' },
      { key: 'minWorkerTokenIssuedAt', value: now, updatedBy: req.user.userId },
      { upsert: true, new: true }
    );
    setMinWorkerTimestamp(now);
    res.json({ message: 'All clinic worker sessions have been successfully logged out from all devices.' });
  } catch (err) {
